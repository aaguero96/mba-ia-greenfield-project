...
/**
 * Supertest types `response.body` as `any`. Naming the shapes this suite reads
 * keeps the assertions type-checked instead of silently unsafe.
 */
interface ErrorBody {
  statusCode: number;
  error: string;
...
describe('Auth (e2e)', () => {
  ...
  async function captureConfirmationToken(
    ...
    jest
      ...
      .mockImplementationOnce((_e: string, _n: string, t: string) => {
        capturedToken = t;
        // Not async: the mock only captures the token, and a resolved
        // promise keeps its signature without a pointless await.
  ...
  describe('JWT Guard', () => {
    ...
    it('returns 200 on GET /auth/me with a valid access token', async () => {
      ...
      expect((res.body as CurrentUserBody).sub).toBeDefined();
      expect((res.body as RegisteredUser).email).toBe('me@example.com');
    });
  ...
  describe('POST /auth/login', () => {
    ...
    it('returns 200 with access_token and refresh_token on valid credentials', async () => {
      ...
      expect((res.body as TokenPair).access_token).toBeDefined();
      expect((res.body as TokenPair).refresh_token).toBeDefined();
      expect(typeof (res.body as TokenPair).access_token).toBe('string');
      expect(typeof (res.body as TokenPair).refresh_token).toBe('string');
    });
  ...
  describe('POST /auth/refresh', () => {
    it('returns 200 with new access_token and refresh_token on valid refresh token', async () => {
      ...
      expect((res.body as TokenPair).access_token).toBeDefined();
      expect((res.body as TokenPair).refresh_token).toBeDefined();
      expect((res.body as TokenPair).refresh_token).not.toBe(refresh_token);
    });
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect((loginRes.body as TokenPair).access_token).toBeDefined();
    });
